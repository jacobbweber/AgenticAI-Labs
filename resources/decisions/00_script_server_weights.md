...
2. **Provider process** (Ollama / vLLM / llama.cpp / a cloud HTTP API). Listens on a port. Tokenizes. Runs the weights. May format or constrain tokens so the response JSON has `message.tool_calls`. [Chapter 00](../../education/00_atoms/00_script_provider_weights.md), [chapter 11](../../education/11_engine_room/00_local_servers.md).
...
- The **model** (via the provider) emits tokens. The provider turns those tokens into a JSON body. If the model chose a tool, that body has `message.tool_calls`. [Chapter 03](../../education/03_the_dispatcher/00_tool_dispatch.md), [chapter 11](../../education/11_engine_room/00_local_servers.md).
