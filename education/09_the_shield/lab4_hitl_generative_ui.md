...
## What you touch
- Script: `lab4_hitl_generative_ui.py`
...
```mermaid
flowchart TD
    subgraph lab4_hitl_script [lab4_hitl_generative_ui.py]
...
```bash
python education/09_the_shield/lab4_hitl_generative_ui.py
```
...
## Stop here
This is not a full React UI. Do not add RBAC or a sandbox in this file. Do not apply a real database migration. Next: [00_fastapi_sse.md](../10_the_front_door/00_fastapi_sse.md).
...
- Contract drift vs `lab4_hitl_generative_ui.py`: pause is `{ "status": "PAUSED", "approval_id", "sdui_frame" }`, not `{ "action": "approval_required", "payload": {} }`. Resume is a second function, not a flipped boolean. The printed `execution_result` is a constant string; no SQL runs. No POST. The intended teaching object is a pause that blocks the write until a human decision. Write that in your copy. Do not edit the `.py` in the repo.
- Chapter 10 is the socket that can stream the pause object to a browser.
