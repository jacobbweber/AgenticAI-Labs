...
## Run
From the repo root. `education/01_the_call/lab1_llm_api_basics.py` is the old reference. It does not define `query_llm`. Write `query_llm` in your copy. Do not invent a second file.

```bash
OLLAMA_HOST=http://127.0.0.1:11434 OLLAMA_MODEL=llama3.2:1b python education/01_the_call/lab1_llm_api_basics.py
```
...
## Stop here
This is not a multi-provider client. Do not add retries, a second host, or a stream parser. Next: [lab2_streaming_tokens.md](./lab2_streaming_tokens.md).
...
- Chapter 11 wraps this call with retries.
