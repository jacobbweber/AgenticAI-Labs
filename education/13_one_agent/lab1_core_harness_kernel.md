...
```bash
OLLAMA_HOST=http://127.0.0.1:11434 OLLAMA_MODEL=llama3.2:1b python education/13_one_agent/lab1_core_harness_kernel.py
```
...
## What you should see
Two `[KERNEL] Starting Turn for Session: 'session_9001'` blocks. Each prints a `[THINKING LOG]` and a `[RESPONSE PAYLOAD]`. The second `response` contains Jacob. `Final State Checkpoint Verified` prints a JSON object with `turn_count: 2`. `education/13_one_agent/state_store/session_9001.json` exists and lists both user lines. If turn 2 does not know the name, `save_state` did not run or turn 2 loaded a new id. If you see `URLError`, the provider is not reachable at the host you set.

## Stop here
Do not treat CoT demux as the point of this lab. Do not add Docker, RBAC, or a second agent. Next: [00_topologies.md](../14_two_agents/00_topologies.md).
...
- Chapter 14 coordinates multiple agents. Chapter 16 adds a sandbox. Chapter 06 owns thinking-token demux. Chapter 20 snaps all pieces onto this kernel.
