# Lab 2: Episodic vs procedural memory
...
## What you touch
- Script: `lab2_episodic_vs_procedural.py` (write it next to this brief; there is no reference `.py` yet)
...
4. Write `route_query(query, facts, procedural_content)`. Lowercase the query. If the query contains a fact `key` or `value` (case-insensitive), return `{ "store": "episodic", "row": that fact }`. Else if the query contains `how` or `step` or a word from the procedural `content` that is not a stop word (`you`, `each`), return `{ "store": "procedural", "content": procedural_content }`. Else return `{ "store": "none" }`.
...
```bash
python education/13_memory/lab2_episodic_vs_procedural.py
```
...
## Notes
- Write `lab2_episodic_vs_procedural.py` next to this brief. There is no reference `.py` in the repo yet.
