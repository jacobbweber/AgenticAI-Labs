...
4. If the work is the same shape and many rows are pending, use many workers on one job table. Volume, not mid-run inspection. See [lab2_two_workers.md](../16_the_job/lab2_two_workers.md).
...
- **When (job table):** volume. Same work, many pending rows. Not mid-run inspection. See [lab2_two_workers.md](../16_the_job/lab2_two_workers.md).
...
4. Job table path (chapter 16 lab 2): many pending rows of the same work. `claim_job(worker)` stores `claimed_by`. A row cannot be claimed twice. No new lab in this folder.
...
- Job table path: [lab2_two_workers.md](../16_the_job/lab2_two_workers.md) - many workers, one file. No new lab in this folder.
...
- **Chapter 16 job table:** many workers on one `jobs.json`. Use for volume, not mid-run inspection.
